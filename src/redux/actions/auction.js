...
import { SHOW_TOAST,HIDE_TOAST,RESET_TOAST } from "../constants/toast";
  ...
  export const createauction = (inputData,signature) => async (dispatch, getState) => {

  
    try {
   
          dispatch({ type: ADD_AUCTION_REQUEST ,payload: inputData });
    
        const {data}  = await axios.post(`${baseUrl}auction/add-auction`, inputData ,
   
        {
          ...
          dispatch({ type:ADD_AUCTION_SUCCESS, payload: data });
      
      } catch (error) {
        dispatch({ type: ADD_AUCTION_FAIL, payload: error.message });
  ...
  dispatch({type:GET_AUCTIONS_SUCCESS,payload:auctions});
       } catch (error) {
...
export const updateauction =(auctionData,id,signature)=>async(dispatch,getState)=>{
...
dispatch({
  ...
  export const deleteauction =(id,signature)=>async(dispatch,getState)=>{
      ...
      try {
        const {data}  = await axios.delete(`${baseUrl}auction/delete-auction/${id}`,{
          headers:{        
