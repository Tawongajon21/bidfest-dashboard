...
export const  developersignin = (email, password) => async (dispatch) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${baseUrl}developer/developer-signin`, {  email, password  });
      ...
      console.log(error);
      dispatch({ type: DEVELOPER_LOGIN_FAIL, payload: error.response.data.msg });
    }
  }
 ...
 export const developerregister = (email,phone,password,name,surname) => async (dispatch) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${baseUrl}developer/developer-signup`, { email,phone,password,name,surname});
